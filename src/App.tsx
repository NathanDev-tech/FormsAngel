...
export default function App() {
  ...
  // BẮT BUỘC: KHÔNG TRƯỜNG NÀO LÀ REQUIRED, LƯU THÀNH CÔNG VÀ XOÁ TRẮNG FORM
  const handleAddMember = async (formData: MemberFormData): Promise<boolean> => {
    try {
      ...
      addToast(
        ...
        `Thông tin ca viên "${nameDisplay}" đã được lưu vào danh sách ca đoàn.`,
        'success'
      );
      return true;
    } catch (err) {
  ...
  const handleSaveEdit = async (id: string, data: Partial<ChoirMember>): Promise<boolean> => {
    try {
      ...
      addToast('Cập nhật thành công', 'Thông tin ca viên đã được cập nhật mới nhất.', 'info');
      return true;
    } catch (err) {
